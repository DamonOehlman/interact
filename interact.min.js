...
g+=f.offsetTop;f=f.offsetParent}while(f)}return{x:h,y:g}}function n(f,h){for(var g=f.target?f.target:f.srcElement;g&&g!==h&&g.nodeName&&g.nodeName.toUpperCase()!="CANVAS";)g=g.parentNode;return g&&g===h}function x(f,h){return{x:f.x-(h?h.x:0),y:f.y-(h?h.y:0)}}function w(f){if(f.preventDefault){f.preventDefault();f.stopPropagation()}else f.cancelBubble&&f.cancelBubble()}var H=[],L=function(f,h,g){g=COG.extend({binder:null,unbinder:null,observable:null},g);for(var a=g.observable,b=null,c=[],d=[],m,j,
o={bind:function(){return a.bind.apply(null,arguments)},pannable:function(v){b=COG.extend({inertia:true},v);return o},unbind:function(){a.unbind();for(var v=0;v<c.length;v++)c[v].unbind();return o}},B=0;B<h.length;B++)c.push(h[B](f,a,g));a.bind("pointerDown",function(){COG.info("reset "+d.length+" pan history");d=[];j=m=0});a.bind("pointerMove",function(v,y,I,z){if(b){d[d.length]={ticks:(new Date).getTime(),x:z.x,y:z.y};a.trigger("pan",z.x,z.y)}m+=z.x?z.x:0;j+=z.y?z.y:0});a.bind("pointerUp",function(v,
y,I){Math.abs(m)>20||Math.abs(j)>20||a.trigger("tap",y,I)});return o};i("pointer",{handler:function(f,h,g){function a(e){if(n(e,f))if(v=m(e)){f.style.cursor="move";w(e);E=e.pageX?e.pageX:e.screenX;D=e.pageY?e.pageY:e.screenY;y=r(E,D);I=q(f);h.trigger("pointerDown",y,x(y,I))}}function b(e){z=e.pageX?e.pageX:e.screenX;F=e.pageY?e.pageY:e.screenY;if(n(e,f))j(v?"pointerMove":"pointerHover")}function c(e){if(v&&m(e)){v=false;if(n(e,f)){f.style.cursor="default";j("pointerUp")}}}function d(e){if(n(e,f)){var A;
e=e||window.event;if(e.detail)A=e.axis===2?-e.detail*o:0;else{A=e.wheelDeltaY?e.wheelDeltaY:e.wheelDelta;if(window.opera)A=-A}if(A!==0){var G=r(z,F);h.trigger("zoom",G,x(G,q(f)),A/B,"wheel");w(e);e.returnValue=false}}}function m(e){e=e||window.event;return(e.which||e.button)==1}function j(e,A,G,M){A=typeof A!="undefined"?A:z;G=typeof G!="undefined"?G:F;var N=A-E,l=G-D,p=r(A,G);h.trigger(e,p,x(p,I),r(N,l));if(typeof M=="undefined"||M){E=A;D=G}}g=COG.extend({},g);var o=120,B=o*8,v=false,y,I,z,F,E,D;
g.binder("mousedown",a,false);g.binder("mousemove",b,false);g.binder("mouseup",c,false);g.binder("dblclick",function(e){COG.info("captured double click");if(n(e,f)){e=r(e.pageX?e.pageX:e.screenX,e.pageY?e.pageY:e.screenY);COG.info("captured double click + target matched");h.trigger("doubleTap",e,x(e,q(f)))}},false);g.binder("mousewheel",d,document);g.binder("DOMMouseScroll",d,document);return{unbind:function(){g.unbinder("mousedown",a,false);g.unbinder("mousemove",b,false);g.unbinder("mouseup",c,
false);g.unbinder("mousewheel",d,document);g.unbinder("DOMMouseScroll",d,document)}}},checks:{touch:false}});i("pointer",{handler:function(f,h,g){function a(l){if(l.count<2)return 0;var p=l.x-l.next.x;l=l.y-l.next.y;return~~Math.sqrt(p*p+l*l)}function b(l,p,t){p=p?p:0;t=t?t:0;for(var K={x:l.x-p,y:l.y-t,id:l.id,count:l.count},J=K;l.next;){l=l.next;J=J.next={x:l.x-p,y:l.y-t,id:l.id}}return K}function c(l,p){var t=l[p?p:"touches"],K,J;if(t.length===0)return null;J=K={x:t[0].pageX,y:t[0].pageY,id:t[0].identifier,
count:t.length};for(var O=1,P=t.length;O<P;O++)J=J.next={x:t[O].pageX,y:t[O].pageY,id:t[O].identifier};return K}function d(l){if(n(l,f)){F=q(f);var p=c(l,"changedTouches"),t=b(p,F.x,F.y);if(!D){E=y;h.trigger("pointerDown",p,t)}M&&h.trigger("pointerDownMulti",p,t);D=c(l);if(D.count>1)A=a(D);N=1;G=b(D)}}function m(l){if(n(l,f)){l.preventDefault();e=c(l);if(E==y)E=Math.abs(D.x-e.x)>B||Math.abs(D.y-e.y)>B?v:y;if(E!=y){E=e.count>1?z:I;if(E==z)if(D.count===1){D=b(e);A=a(D)}else{l=a(e);if(Math.abs(A-l)<
o)E=I;else{var p;p=e.x;var t=e.next.x,K=e.y,J=e.next.y;p={x:(p<t?p:t)+(Math.abs(p-t)>>1),y:(K<J?K:J)+(Math.abs(K-J)>>1)};l=l/A;t=l-N;h.trigger("zoom",p,x(p,F),t,"pinch");N=l}}E==I&&h.trigger("pointerMove",e,b(e,F.x,F.y),r(e.x-G.x,e.y-G.y));M&&h.trigger("pointerMoveMulti",e,b(e,F.x,F.y))}G=b(e)}}function j(l){if(n(l,f)){var p=c(l,"changedTouches"),t=b(p,F.x,F.y);e=c(l);if(!e){E===y&&h.trigger("pointerTap",p,t);h.trigger("pointerUp",p,t);D=null}M&&h.trigger("pointerUpMulti",p,t)}}g=COG.extend({detailed:false,
inertia:false},g);var o=20,B=7,v=0,y=1,I=2,z=3,F,E,D,e,A,G,M=g.detailed,N=1;g.binder("touchstart",d,false);g.binder("touchmove",m,false);g.binder("touchend",j,false);COG.info("initialized touch handler");return{unbind:function(){g.unbinder("touchstart",d,false);g.unbinder("touchmove",m,false);g.unbinder("touchend",j,false)}}},checks:{touch:true}});return{register:i,watch:function(f,h,g){h=COG.extend({bindToBody:false,observable:null,isIE:typeof window.attachEvent!="undefined",types:null},h);capabilities=
COG.extend({touch:"ontouchstart"in window},g);if(!h.observable){COG.info("creating observable");h.observable=COG.observable({});globalOpts=h}h.binder=h.isIE?k:u(h.bindToBody);h.unbinder=h.isIE?s:C(h.bindToBody);g=h.types;for(var a=capabilities,b=[],c=H.length;c--;){var d=H[c],m=!g||g.indexOf(d.type)>=0,j=true,o;for(o in d.checks){var B=d.checks[o];COG.info("checking "+o+" capability. require: "+B+", capability = "+a[o]);j=j&&B===a[o]}if(m&&j)b[b.length]=d.handler}return new L(f,b,h)}}}();
